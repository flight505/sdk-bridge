#!/bin/bash
# validate-result.sh — SubagentStop hook for SDK-Bridge
...
# Skip validation if implementer reported story as skipped
if [ "$IMPL_STATUS" = "skipped" ]; then
  exit 0
fi

# Find project config — SDK-Bridge uses JSON config
PROJECT_DIR="$(pwd)"
...
# Exit 2 blocks the agent; reason goes to stderr
